# Draft - Ilia Karmanov - 18/10/2016
#
...
# Routing
# Use OSRM instead of Bing to get Cycling
library(osrm)
...
for (i in 1:nrow(cycleJourniesSum)) {
  ...
  from <- c(paste0(i, "a"),
            cycleJourniesSum[i ,"start.lon"],
            cycleJourniesSum[i ,"start.lat"])
  to <- c(paste0(i, "b"), 
          cycleJourniesSum[i ,"end.lon"],
          cycleJourniesSum[i ,"end.lat"])
...
# Save & Load
write.csv(mergedPaths, 'route_data2.csv', row.names=FALSE)
mergedPaths <- read.csv('route_data2.csv')
